...
public int numRelays = 3; //how many relays are needed
ReentrantLock lock = new ReentrantLock();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        device_ip = findViewById(R.id.textView3);
    ...
    public void dirConnect(){
        String portMsg = "€PORT:" + Source_Port;
            ...
            if(dest_ip == null){runOnUiThread(()-> Toast.makeText(getApplicationContext(),"Choose destination",Toast.LENGTH_SHORT).show());}
                ...
                try {
                    ...
                    if (access_arr == 1) {    //if requesting connection or update from directory, update node list
                        ...
                        String ACK = inputSend.readLine();
                    }
                } catch (IOException e) {
                    if(access_arr == 2){    //remove node
                        removeNode(dest_ip, Integer.toString(dest_port));
                        updateNodeList();   //tell arrayadapter (on-screen list) to update
                    }
    ...
    //update list of nodes used for displaying to listview/arrayadapter UI element
    ...
    public String msgConfig(){
        ...
        dirUpdate(view);    //called to get up to date list
        ...
        //finally add destination address info and the message to be received
        relays.add(Dest_IP);
    ...
    public String getLocalIpAddress() {
        try {
            ...
            runOnUiThread(()-> Toast.makeText(getApplicationContext(),"Error retrieving Local IP address",Toast.LENGTH_SHORT).show());
        }
        return null;
    }
